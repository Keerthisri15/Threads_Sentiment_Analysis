...
Original file is located at
    https://colab.research.google.com/drive/1RNmUJ7yA1tErkeA5w0T6LEFxXkckNPSX
"""
...
"""Count plot is for limited number of data only"""
...
"""autopct :upto how many decimal places we are going to show the distribution"""
...
"""Here the Distribution is not Uniform

here we have to convert the date from object to date time object using pandas library
"""

df['review_date'] = pd.to_datetime(df['review_date'])

"""Now on this Date and Time we are performing Timeseries analysis"""
...
"""This dataset is of 20 days Data

The below code means that we are extracting reviwes for 1 week<br>
i.e. we are getting weekly data
"""

week_delta = pd.Timedelta(days=6)
current_date = start_date

"""week delta is the cutoff of week ending<br>
if week delta = 6 we will get 7 days data
"""
...
"""We are resampling on daily basis"""
...
"""here we are organizing the data by source(i.e how many from appstore and how many from googleplay store) by using group by<br>
...
"""here we are just changing the marker that's it<br>
grouping is the only important line of code<br>
remaining we can change what we want
"""
...
"""From this we can observe that daywise the reviews are getting decreased"""

fig = go.Figure()
for source_name, source_data in df1.groupby('source'):
    fig.add_trace(go.Scatter(x=source_data.resample('D').size().index, y=source_data.resample('D').size().values,
                            mode='markers+lines', name=source_name))
fig.update_layout(title='Number of Reviews Day-wise',
                  xaxis_title='Date',
                  yaxis_title='Number of Reviews',
                  xaxis_tickangle=-45)
...
"""Here we can use only lines ,only markers or both lines + markers also"""
...
"""Here we are doing daily wise ratings"""
...
"""Here the reviews are positive(1),negative(-1),neutral(0)"""
...
"""Here we have drawn only plotly only if we want we can draw with matplotlib and other libraries also.

## **Data Preprocessing**

Here we are performing text cleaning.
"""
...
"""Here we are removing Punctuation marks"""
...
"""w+ means words having only alphanumeric<br>
W+ means means which are not alphanumeric(w+)

Here we have already removed the non -alpha numeric characters(like punctuation marks) so there will be no change in it.
"""
...
"""Here we are applying the remove stop words functions also but there is no change."""
...
Here we are removing only a particular Regular Expression only there may be alot of patterns
"""
...
"""here we are again tokenizing our words and checking the stopwords,if it is astopword we don't consider it.if it is not a stop words then lemmatize it.<br>
we are performing again because we don't have trust on
"""

df_new.review_description =  df_new.review_description.apply(lambda x: lemmatizer(x))

"""Here the texts got Lemmatized"""

df_new.review_description

"""Here we are replacing the url's with empty space<br>
multiline checks for the url's in multi lines
"""
...
"""Here we are again splitting into words and checking if there are any digits and replace them by white space

by using the above function we can remove the digits but if there are any numbers between letters we cannot use this function.<br>so we are using again using the below function.
"""
...
"""Here we are removing emojis present in it.<br>
there are alot of patterns but here we are using only some patterns.
"""
...
"""Word cloud is mandatory in nlp projects."""
...
"""The above word cloud is for overall

collocation means similar words<br>
collocation = False means it takes only unique words(it doesn't consider the words with similar meanings)<br>
"""
...
"""The above word cloud is for reviews where rating == 1"""
...
"""The above word cloud is for negative sentiments

Try creating custom word clouds
"""
...
"""fit transform fits the model and converts to numerical values as computers don't undersatnd categorical values

we can use bag of words or count vectorizer or bag of words also
"""

data_vec

"""TF - IDF creates matrix of numerics(float values)

Here we are saving our tf_idf model using a pickle file<br>
so that we can use it later.<br>
pickle is used to save our file in dump formart.The files present in dump cannot be seen by us only the system can know the contents of this.<br>
These saved files will be used for deployment.
"""
...
y=df_new['rating'].values   #ratings are represented on y axis(1,0,-1)
...
"""Ada boost is called as adaptive boosting technique.it is a ensemble model<br>
boosting algorithms are used to increase the performance of models
"""
